...
namespace StudentCounter
{
    public class StudentCounter
    {
        public static void Main(string[] args)
        {
            ...
            double ageSum = 0;
            Console.WriteLine("The people who have more than 4 candies are:");

            for (int i = 0; i < map.Count; i++)
            {
                if (Convert.ToInt32(map[i]["candies"]) > 4)
                {
                    Console.WriteLine(map[i]["name"]);
                }
                if (Convert.ToInt32(map[i]["candies"]) < 5)
                {
                    ageSum += Convert.ToDouble(map[i]["age"]);
                }
            }
                Console.WriteLine("The sum of the ages of the people who have less than 5 candies is:");
